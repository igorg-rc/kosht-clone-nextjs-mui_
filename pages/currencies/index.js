  ...
  return <>
    ...
    <Table>
      ...
      <TableBody>
        {items
          .filter(row => {
            let requestedField = router.locale === "uk" ? row.title_ua : row.title_en
            if (query === "") {
              return row
          ...
          .map(item => (
            <TableRow key={item.id}>
              <TableCell style={{ textAlign: "left" }}>
                <Image src={item.icon} className={styles.bankIcon} style={{ marginRight: 30 }} />
                <span className={styles.bankTitle}>{router.locale === "uk" ? item.title_ua : item.title_en}</span>
              </TableCell>
...
export default function CurrencyFull() {
  const router = useRouter()
  const types = router.locale === "uk" ? exchangeListUA : exchangeListEN
